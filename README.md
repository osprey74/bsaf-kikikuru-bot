...
bsaf-kikikuru-bot monitors the Japan Meteorological Agency (JMA) public XML feed for the new "R06" weather warnings and advisories — telegrams VPWW55–61, operational since 2026-05-29 — designated-river flood forecasts (VXKO), weather disaster-prevention briefings (VPBS50: linear precipitation bands, significant snowfall), and typhoon analysis/forecast information (VPTW60), automatically posting them to Bluesky with structured BSAF tags. BSAF-compatible clients can filter posts by phenomenon, alert level, and prefecture.
...
Designated-river flood forecasts (VXKO) are published as 1 post = 1 prefecture × 1 alert level, with the affected river names listed in the post body. VPBS50 linear precipitation bands and significant snowfall are published as `type:linear-rainband-warning` / `type:significant-snowfall-warning`. VPBS50 record short-term heavy rain is out of scope for now (already published by bsaf-jma-bot via VPOA50). Typhoon information (VPTW60) is published as `type:typhoon` with `value:` intensity class (violent/very-strong/strong/tropical-storm) and `target:jp` (nationwide). Storm-zone entry probability (VPTA50) is out of scope for now.
