...
bsaf-kikikuru-bot は気象庁の防災情報XMLフィードを監視し、新気象警報・注意報（Ｒ０６）電文（VPWW55〜61、2026-05-29 運用開始）・指定河川洪水予報（VXKO）・府県気象防災速報（VPBS50：線状降水帯・顕著な大雪）を BSAF タグ付きで Bluesky に自動投稿します。BSAF 対応クライアントでは、現象種別・警戒レベル・都道府県によるフィルタリングが可能です。
...
指定河川洪水予報（VXKO）は 1 投稿 = 1 都道府県 × 1 警戒レベルで配信し、対象河川名を本文に列挙します。VPBS50 の線状降水帯・顕著な大雪は `type:linear-rainband-warning` / `type:significant-snowfall-warning` で配信します。なお VPBS50 の記録的短時間大雨は bsaf-jma-bot（VPOA50）が配信中のため、当面本 Bot では対象外です。
