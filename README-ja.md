...
bsaf-kikikuru-bot は気象庁の防災情報XMLフィードを監視し、新気象警報・注意報（Ｒ０６）電文（VPWW55〜61、2026-05-29 運用開始）・指定河川洪水予報（VXKO）・府県気象防災速報（VPBS50：線状降水帯・顕著な大雪）・台風解析予報情報（VPTW60）を BSAF タグ付きで Bluesky に自動投稿します。BSAF 対応クライアントでは、現象種別・警戒レベル・都道府県によるフィルタリングが可能です。
...
| VXKO50〜70 | 指定河川洪水予報 | Lv5氾濫発生 / Lv4氾濫危険 / Lv3氾濫警戒 / Lv2氾濫注意 ＋ 解除 |
| VPBS50 | 府県気象防災速報 | 線状降水帯（直前予測=Lv4相当 / 発生=Lv5相当）/ 顕著な大雪（Lv4相当） |
| VPTW60 | 台風解析・予報情報 | 台風の実況（番号・強さ・中心気圧・最大風速・位置・進路・暴風域） |
| VPFJ50 | 府県気象情報 | 警報前段の見通し・解説（大雨・落雷・高温 等の平文情報） |
| VPHW50/51 | 竜巻注意情報 | 竜巻など激しい突風の注意喚起（bsaf-jma-bot から移管） |
| VXWW50 | 土砂災害警戒情報 | 都道府県＋気象台の共同発表（警戒レベル4相当・bsaf-jma-bot から移管） |
| VPBS50 | 記録的短時間大雨 | 猛烈な雨の実況（bsaf-jma-bot から移管、VPBS50 サブ種別） |
<!-- markdownlint-enable MD060 -->

BSAF の「情報は可能な限り多く提供し、取捨選択はクライアント利用者に委ねる」という設計思想に従い、**全現象・全警戒レベル・全警報級・解除をすべて配信**します。

指定河川洪水予報（VXKO）は 1 投稿 = 1 都道府県 × 1 警戒レベルで配信し、対象河川名を本文に列挙します。VPBS50 の線状降水帯・顕著な大雪は `type:linear-rainband-warning` / `type:significant-snowfall-warning` で配信します。なお VPBS50 の記録的短時間大雨は bsaf-jma-bot（VPOA50）が配信中のため、当面本 Bot では対象外です。台風情報（VPTW60）は `type:typhoon`・`value:`（強さ階級 violent/very-strong/strong/tropical-storm）・`target:jp`（全国）で配信します。暴風域に入る確率（VPTA50）は当面対象外です。

**中小河川の浸水想定詳細（VXSUii）と Lv1 早期注意情報（VPFD61 / VPFW60）は現在対象外**です（将来別 Bot で対応予定）。
